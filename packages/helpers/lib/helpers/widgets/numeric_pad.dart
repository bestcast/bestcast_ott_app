...
class _NumericInheritedWidget extends InheritedWidget {
  ...
  static _NumericInheritedWidget of(BuildContext context) {
    final _NumericInheritedWidget? result = context.dependOnInheritedWidgetOfExactType<_NumericInheritedWidget>();
    assert(result != null, 'No _NumericInheritedWidget found in context');
    return result!;
  }

  @override
  bool updateShouldNotify(_NumericInheritedWidget old) => style != old.style || onTap != old.onTap;
}
    ...
    return SplashTap(
      ...
      child: Container(
        ...
        child: number.child ??
            Text(
              number.title,
              style: numeric.style.style ?? context.textTheme.bodyLarge ?? const TextStyle(),
            ),
      ),
    );
  }
